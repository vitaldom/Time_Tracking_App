...
public class JdbcAbstractUserDao implements UserDao {
    ...
    @Override                                 //TODO extract to interface?
    public List<AbstractAppUser> findAll() {
        List<AbstractAppUser> users = new ArrayList<>();
        ...
        try (PreparedStatement statement = connection.prepareStatement(FIND_ALL_USERS)) {
            ...
            LOGGER.error("Error getting list of users from database");
            exception.printStackTrace();
        }
        return users;
    }
    ...
    @Override
    public User findClientByLogin(String login) {                   //TODO consider deleting
        ...
        try(PreparedStatement ps = connection.prepareStatement(FIND_CLIENT_BY_LOGIN)) {
            ...
            ResultSet rs = ps.executeQuery();
            while (rs.next()) { //TODO+ duplicate values possible?
                User user = (User) userMapper.extractFromResultSet(rs);
                return user;
            }
        } catch (SQLException exception) {
            LOGGER.error("Error while getting a user from database");
    ...
    public AbstractAppUser findById(int id) {
        AbstractAppUser user = null;

        try (PreparedStatement ps = connection.prepareStatement(FIND_APP_USER_BY_ID)) {
            ps.setInt(1, id);
            ResultSet rs = ps.executeQuery();
            if (rs.next()) {
            ...
            LOGGER.error("Error while getting user by ID from database");
            exception.printStackTrace();
        }
